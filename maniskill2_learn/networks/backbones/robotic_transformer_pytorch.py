...
class RT1(nn.Module):
    ...
    def forward(
        ...
        video = self.transform(video["rgb"][:,:3,:,:].float()/255).unsqueeze(2)
        # video = torch.nn.functional.interpolate(video, (1, 224,224))
        texts = ["Pick up a cuboid-shaped peg and inserting it into a hole"] * video.shape[0]
        frames, device = video.shape[2], video.device
